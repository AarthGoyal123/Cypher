...
def test_build_retrieval_text_standard():
    row = pd.Series({
        'business_name_norm': 'apple inc', 
        'business_address_norm': '1 infinite loop'
    })
...
def test_build_retrieval_text_missing_address():
    row = pd.Series({
        'business_name_norm': 'apple inc', 
        'business_address_norm': float('nan')
    })
    expected = "business name: apple inc address: "
    assert build_retrieval_text(row) == expected

def test_build_retrieval_text_deterministic():
    row1 = pd.Series({'business_name_norm': 'a', 'business_address_norm': 'b'})
    row2 = pd.Series({'business_name_norm': 'a', 'business_address_norm': 'b'})
